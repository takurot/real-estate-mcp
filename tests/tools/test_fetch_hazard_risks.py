...
class TestFetchHazardRisksTool:
    ...
    async def test_fetch_multiple_risks(self, tool, mock_http_client):
        ...
        flood_data = {
            ...
            "features": [
                {"type": "Feature", "properties": {"rank": "rank_3"}}
            ]
        }
        landslide_data = {
            "type": "FeatureCollection",
        ...
        # x approx 29105, y approx 12902 (Need to verify calc if strict, but mock just checks call args exist)
    ...
    async def test_partial_failure(self, tool, mock_http_client):
        ...
        async def side_effect(dataset_id, **kwargs):
            if dataset_id == "XKT026":
                ...
                return FetchResult(data={"features": [{"properties": {"type": "steep_slope"}}]})
            return FetchResult()
